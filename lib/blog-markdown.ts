...
const FIRST_BODY_HEADING = 2;

function minMarkdownHeadingLevel(lines: string[]): number {
  let min: number | null = null;
  for (const line of lines) {
    const match = /^(#{1,6})\s+/.exec(line.trim());
    if (!match) continue;
    const level = match[1].length;
    min = min === null ? level : Math.min(min, level);
  }
  return min ?? FIRST_BODY_HEADING;
}
...
function resolveHeadingLevel(
  ...
  let level = rawLevel - minLevel + FIRST_BODY_HEADING;
  level = Math.min(6, Math.max(FIRST_BODY_HEADING, level));
...
export function markdownToSafeHtml(markdown: string): string {
  ...
  const minHeadingLevel = minMarkdownHeadingLevel(lines);
  const parts: string[] = [];
